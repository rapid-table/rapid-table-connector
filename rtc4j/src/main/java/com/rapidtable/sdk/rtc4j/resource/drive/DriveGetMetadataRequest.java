...
public class DriveGetMetadataRequest implements IRequest {
    private final String path;
    private final String query;

    DriveGetMetadataRequest(final String path, final String query) {
        this.path = path;
        this.query = query;
    }
    ...
    public static class Builder {
        ...
        public DriveGetMetadataRequest build() {
            final var path = PathConfig.ROOT + PathConfig.WORKSPACE + String.format("/%s", workspaceId) +
                PathConfig.DRIVES + PathConfig.METADATA + String.format("/%s", objectId);
            return new DriveGetMetadataRequest(path, null);
        }
