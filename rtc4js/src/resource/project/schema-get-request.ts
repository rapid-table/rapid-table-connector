...
export class SchemaGetRequest implements IRequest {
...
class Builder {
    ...
    public build(): SchemaGetRequest {
        const path = PathConfig.ROOT + PathConfig.WORKSPACE + `/${this._workspaceId}` +
            PathConfig.PROJECTS + `/${this._projectId}` + PathConfig.SCHEMA;

        return new SchemaGetRequest(path, {});
    }
